...
public class IndexAction extends BaseAction {
	...
	public String header(UserAgent user, Model model) {
		...
		List<SubSystem> sysList = this.subSystemService
				.getSubSystemByUserId(Long.valueOf(user.getId()));
		if (sysList == null) {
			sysList = new ArrayList();
		}

		List _sysList = this.roleSystemDAO.selectSystemByUserId(Long
				.valueOf(user.getId()));
	...
	@AdminAccess(value = {})
	public String menu(UserAgent user, Model model,
			@RequestParam(value = "subSysId", required = false) Long subSysId,
			Cookyjar cookyjar) {
		if (user == null) {
			return "system/login";
		}
		...
		this._log.debug("user.getCurrentSystemCode()===="
				+ user.getCurrentSystemCode());
		if (currSysCode != null) {
			this.cookieWriter.updateUserAgentPermissionBySubsystem(user,
					cookyjar, currSysCode);
		}
		...
		if (StringUtils.isNotBlank(currSysCode)) {
			...
			if (user.isSuperUser()) {
				authList = this.authorityService.selectMenuBySubsystem(subSys
						.getId());
			} else {
				authList = this.authorityService.selectMenuByUserAndSubsystem(
						Long.valueOf(user.getId()), currSysCode);
			}

			if (authList != null) {
				List menuList = new ArrayList();
				for (Authority auth : authList) {
					if (auth.getType().shortValue() == EnumAuthorityType.MENU_BAR
							.getCode()) {
						List childList = new ArrayList();
						for (Authority child : authList) {
							if ((child.getType().shortValue() == EnumAuthorityType.MENU
									.getCode())
									&& (auth.getId().longValue() == child
	...
	public String body(Model model, UserAgent user) {
		SysConfig con = this.sysConfigService
				.selectSysConfigByCode("system_name");
		if (con != null) {
			model.addAttribute("systemName", con.getValue());
		}
		if (StringUtils.isNotBlank(user.getCurrentSystemCode())) {
			model.addAttribute("subSys", this.subSystemService
					.selectByCode(user.getCurrentSystemCode()));
		}
		if ((user.getEclpLastLoginTime() != null)
				&& (user.getEclpLastLoginTime().longValue() != -1L)) {
			model.addAttribute("lastLoginTime", DateUtil.convertDateToString(
					"yyyy-MM-dd HH:mm:ss", new Date(user.getEclpLastLoginTime()
							.longValue())));
		}
