var mainPlatform = {
	...
	init: function(){
		...
		$("#pf-title").html($('.pf-nav > .current').get(0).title);
	},
	...
	bindEvent: function(){
        ...
        $(document).on('click', '.pf-logout', function() {
            layer.confirm('您确定要退出吗？', {
			  ...
			  location.href= appServer + "/system/logout.htm"; 
			});
        });
        //左侧菜单收起
	...
	renderByURL:function(url,title){
		$.ajax({
		   type: "GET",
		   url: url,
...
function addTab(title,url){
	if ($('#pf-tabs').tabs('exists', title)){
		$('#pf-tabs').tabs('select', title);
	} else {
