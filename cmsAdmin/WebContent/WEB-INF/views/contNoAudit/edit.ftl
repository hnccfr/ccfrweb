...
function optRevocation(){
	if(confirm('确认执行撤销操作吗？')) {
		window.location.href='${appServer.get('/contNoAudit/revocation.htm?id=${cont.id}&q=${q}')}'
	}
}
			...
			-->
				...
				<tr>
					...
					<#-- 增加当编辑状态即可以提交，解决终审人员保存后无法提交	sunjin	20130715 -->
						<input id="buttonSubmit" type="button" class="button-s4" value='保存并发布' onclick="$('#sortDateStr').val($('#sortDate').val());$('#draft').val('false');$('#jvForm').submit();"/> &nbsp;
					<#if cont.status != 1>
						<input type="button" onclick="optReject()" class="button-s4" value="退回"/>
					</#if>
...
<div id="rejectDialog" title="文章退回">
	<form method="post" id="rejectForm" action="${appServer.get('/contNoAudit/reject.htm')}">
