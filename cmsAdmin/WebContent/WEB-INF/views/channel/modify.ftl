...
-->
  <div class="formBox mt10">
    <h3><i></i><span>栏目修改</span></h3>
    <form action="${appServer.get('/channel/update.htm')}" id="updateForm" method="POST">
		      	...
		      	<#if channel.parentId??>
			      	<#if channelList??>
			      		<#list channelList as c>
			      			<#if c.id == channel.parentId>${c.channelName}</#if>
			      		</#list>
					</#if>
				<#else>
					顶级栏目
		      	...
		      	<!--
		      		<select name="parentId">
		      			...
		      			<#if channelList??>
		      			<#list channelList as c>
		      				<option value="${c.id}" <#if channel.parentId?? && c.id == channel.parentId>selected="selected"</#if> >
		      				<#if c.level gt 1>
		      ...
		      <tr <#if item.isDisplay?? && item.isDisplay==2> style="display:none;" </#if>>
		      	...
		      	<#if item.isCustom == 0><!-- 系统预定义字段 -->
			      	...
			      	<#elseif item.field="code"><!-- 访问路径 -->
			      		<#if channel.code??>${channel.code}</#if>
			      		<input type="hidden" name="code" value="<#if channel.code??>${channel.code}</#if>" />
			      		<#--
    ...
    <#if settlerAgent.access("PermissionEnum.CHANNEL_EDIT")>
      <button id="buttonSave" type="button" class="button-s4" onclick="doSubmit()">修改</button>
     </#if>
