...
<div class="main">
	<div class="rhead">
  	...
  	<div class="formBox mt10">
    	<h3><i></i><span>友情链接修改</span></h3>
    	<form action="${appServer.get('/link/edit.htm')}" id="inputForm" method="POST">
    		<input type="hidden" name="q" value="<#if q??>${q}</#if>" />
    		<input name="id" type="hidden" value="<#if link.id??>${link.id}</#if>"/>
	    	<div class="content">
	      		<table class="c2">
		     		<tr>
			      		<th><@s.m "cmsFriendlink.name"/>：</th>
			      		<td>
			      			<input name="name" type="text" maxlength="20" class="inpt" value="<#if link.name??>${link.name}</#if>" />
		      				<span class="red">*</span>
		     		...
		     		<tr>
		      			...
		      			<td>
		      				<input name="url" type="text" maxlength="200" class="inpt" value="<#if link.url??>${link.url}</#if>" />
							<span class="red">*</span>
							<span class="error"></span>
		      			</td>
		     		</tr>
		     		...
		     		<tr>
		     			<th colspan="2">
			     			<img noresize="noresize"
			     				...
			     				alt="预览"
			     				src="<#if link.logoAbsolute??>${link.logoAbsolute}</#if>"
			     				id="preImg" />
		     			</th>
		     		</tr>
		     		<tr>
		      			<th>简介：</th>
		      			<td>
		      				<textarea cols="10" rows="3" name="remark" maxlength="20" class="w200"><#if link.remark??>${link.remark}</#if></textarea>
							<span class="error"></span>
		      			</td>
		     		</tr>
		     		<tr>
		      			...
		      			<td>
		      				<input name="priority" type="text" maxlength="3" class="inpt" value="<#if link.priority??>${link.priority}</#if>" />
							<span class="red">*</span>
							<span class="error"></span>
		      			</td>
		     		</tr>
