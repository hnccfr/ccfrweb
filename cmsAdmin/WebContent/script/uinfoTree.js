$(function() {
	$("input[name='showType']").click(function() {
		alert();
		window.location.href = appServer + "/userInfo/tree.htm?showType=" + this.value
			+ "&showUsing=" + $("input[name='showUsing']:checked").val();
   	});
	$("input[name='showUsing']").change(function() {
		window.location.href = appServer + "/userinfo/tree.htm?showUsing=" + this.value 
...
//	alert(treeNode.id);
	var ids = treeNode.id;
	parent.authMainIframe.location.href = appServer + "/userinfo/main.htm?showType=" + ids;
}
