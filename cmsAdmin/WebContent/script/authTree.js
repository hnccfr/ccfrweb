$(function() {
	...
	$("input[name='showUsing']").change(function() {
		window.location.href = appServer + "/auth/tree.htm?showUsing=" + this.value 
...
function addHoverDom(treeId, treeNode) {
	...
	if (treeNode.pId != -1 && treeNode.define == 2 ) {
		...
		if (aObj) aObj.bind("click", function(){
			parent.authMainIframe.location.href = appServer + "/auth/view.htm?id=" + treeNode.id;});
	}
}
...
function doDel(){
	...
	if( !confirm( "您确定要删除这些权限吗？\n删除后无法再找回，且所有人员将不再具有这些权限！" ) ){
		return;
	}
	var ids = "";
	...
	parent.authMainIframe.location.href = appServer + "/auth/del.htm?id=" + ids;
}

function doAdd(){
	parent.authMainIframe.location.href = appServer + "/auth/add.htm";
}
...
function doHelp(){
	parent.authMainIframe.location.href = appServer + "/auth/list.htm";
}
