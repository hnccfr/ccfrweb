...
Import(FCK.Plugins.Items['Media'].Path + 'js/fck_media_inc.js');
...
if ( oFakeImage )
{
	if ( oFakeImage.tagName == 'IMG' && oFakeImage.getAttribute('_fckmedia') ) {
		...
		if(oEmbed.childNodes.length>0) {
			oEmbed = oEmbed.childNodes[oEmbed.childNodes.length-1];
		}
	} else {
		oFakeImage = null ;
	}
...
function OnUploadCompleted( errorNumber, fileUrl, fileName, customMsg )
{
	switch ( errorNumber )
	{
		...
		case 101 :	// Custom warning
			...
			break ;
		case 201 :
			alert( '存在同名文件，此次上传的文件已被重命名为：“' + fileName + '”' ) ;
			break ;
		case 202 :
			alert( '文件类型无效' ) ;
			return ;
		case 203 :
			alert( "安全错误，没有上传权限" ) ;
			return ;
		case 204 :// 添加上传文件过大错误代码 zhoujianbin 20120821
			alert( "上传文件过大" ) ;
			return ;
		case 500 :
			alert( '网络链接超时' ) ;
