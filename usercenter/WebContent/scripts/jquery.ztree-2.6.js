...
//			alert( node.tId + "	" + node.isParent );
	...
	//根据类型显示相应icon	sunjin	2011-08-19
	function changeIcon( setting, node ){
		if( setting.iconTypes != undefined ){
			if( node.type != undefined ){
				for( var j=0; j<setting.iconTypes.length; j++ ){
					if( node.type == setting.iconTypes[ j ].type ){
						node.icon 		= setting.iconTypes[ j ].icon;
						node.iconOpen 	= setting.iconTypes[ j ].iconOpen;
						node.iconClose 	= setting.iconTypes[ j ].iconClose;
						break;
					}
				}
			}
	...
	function asyncGetNode(setting, treeNode) {
		if( treeNode ){
//			alert( "in asyncGetNode!!!treeNode = " + treeNode + "|treeNode.isAjaxing = " + treeNode.isAjaxing + "|treeNode.isParent = " + treeNode.isParent );
		}
		var i, l;
...
//		alert( "continue!!!" );
	...
	//	alert( tools.apply(setting.asyncUrl, [treeNode], setting.asyncUrl) );
		$.ajax({
			type: "POST",
...
//						alert( "string	in asyncGetNode!!! msg = " + msg );
						newNodes = eval("(" + msg + ")");
					} else {
//						alert( "other	in asyncGetNode!!! msg = " + msg );
						newNodes = msg;
					}
				} catch(err) {}
...
//				alert( "in asyncGetNode!!! newNodes = " + newNodes );
//				for(i=0; i<newNodes.length; i++){
//					alert( "in asyncGetNode!!!	newNodes[ " + i + " ].pId = " + newNodes[ i ].pId
//							 + "	newNodes[ " + i + " ].isParent = " + newNodes[ i ].isParent
//							 + "	newNodes[ " + i + " ].parent = " + newNodes[ i ].parent) ;
//					newNodes[ i ].isParent = newNodes[ i ].parent;
//				}
...
//		alert( "in addTreeNodes!!! setting.isSimpleData = " + setting.isSimpleData );
...
//		alert( "in addTreeNodesData!!!parentNode.id = " + parentNode.id );
//		alert( "in addTreeNodesData!!!treenodes = " + treenodes );
...
//		alert( "in transformTozTreeFormat!!!" );
		var i,l;
		var key = setting.treeNodeKey;
		var parentKey = setting.treeNodeParentKey;
//		alert( "in transformTozTreeFormat!!! key = " + key + "	parentKey = " + parentKey );
