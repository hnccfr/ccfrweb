...
$(document).ready(function() {
	...
	//loadPrjTradeTypeFormURL(tType, "prjTradeTypeBoxId", "");// 加载交易动态属性
	loadPrjTradeTypeDisByBsType(tType, "prjTradeTypeBoxId", "",bsType);// 加载交易动态属性,带挂牌方向参数
				...
				function() {
					jQuery.validator.setDefaults( {
						submitHandler : function(form) {
							...
							//		+ $("#address").val();
							var strAddress = pe_province.text()
							+ (pe_city.text() != "请选择" ? pe_city.text() : "") + (pe_area.text() != "请选择" ? pe_area.text() : "")
							+ $("#address").val();
...
/**
 * ajax删除之前上传的附件
 * @param projectId 挂牌项目Id
 * @param attachedFilePath 挂牌项目附件路径
 * @return
 */
